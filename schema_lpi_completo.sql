...
-- ⚠ SNAPSHOT HISTÓRICO (junio 2026). En producción las contraseñas ya NO se guardan
--   en texto plano: van con hash bcrypt (pgcrypto) y el login pasa por `staff_login`.
--   No correr esto tal cual. La contraseña real está en CLAVES_ERP_LPI.txt (fuera de git).
INSERT INTO usuarios_sistema (usuario, pass, nombre, rol)
VALUES ('admin', '<<CONTRASEÑA — ver CLAVES_ERP_LPI.txt>>', 'Administrador', 'Dueño / Director General')
...
-- ── CLAVES DE PADRES (acceso a calificaciones) ───────────────
CREATE TABLE IF NOT EXISTS claves_padres (
  id          SERIAL PRIMARY KEY,
...
ALTER TABLE claves_padres ENABLE ROW LEVEL SECURITY;
CREATE POLICY "acceso_total" ON claves_padres FOR ALL USING (true) WITH CHECK (true);
CREATE INDEX IF NOT EXISTS idx_claves_clave ON claves_padres(clave);
